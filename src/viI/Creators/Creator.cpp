...
Coordinates calculateCoordinates(const Vector& vector) {
    // Convert angle from degrees to radians
    float angleRad = vector.angle * (M_PI / 180.0);

    // Calculate x and y coordinates using trigonometry
    float x = vector.magnitude * std::cos(angleRad);
    float y = vector.magnitude * std::sin(angleRad);
...
void Creator::createRadial(int n, int m)
{
    ...
    for(int j=0; j<m;j++){
        ...
        Vector v = {(1000000.0f * n) / 2, j*delta};
        Coordinates coordinates = calculateCoordinates(v);

        Line line(0,0,0,coordinates.x,coordinates.y,0, local_options);
        plProcessor.transform(line);
       scene->addChild(true, line.transform);
    }
